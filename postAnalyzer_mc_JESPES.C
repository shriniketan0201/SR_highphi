...
void postAnalyzer::Loop(Long64_t maxEvents, int reportEvery)
{
  ...
  double nPhoCand_weighted, nMET170_weighted, nDphiPhoMET_weighted, nLeptonVeto_weighted, nDphiJetMET_weighted, nPhoEtOverMET_weighted;
  nPhoCand_weighted = nMET170_weighted = nDphiPhoMET_weighted = nLeptonVeto_weighted = nDphiJetMET_weighted = nPhoEtOverMET_weighted = 0.0;
  
  int nPhoPToverMET_POS, nPhoPToverMET_NEG;
  nPhoPToverMET_POS = nPhoPToverMET_NEG = 0;
  ...
  // TH1D *ewkCorrection = (TH1D*)file->Get("znng");
  ...
  for (Long64_t jentry=0; jentry<nentriesToCheck;jentry++)
  {
	...
	//  Double_t EWK_percent_adjustment = ewkCorrection->GetBinContent(ewkCorrection->GetXaxis()->FindBin(uncorrectedPhoEt));
      ...
      Float_t uncorrectedPhoEt = ((*phoEt)[phoCand[0]]);
        event_weight = 1.0;
	//below four lines are trying
	Double_t phweight = 1.0;
	Double_t phsys=0.0;
	//        if( uncorrectedPhoEt >= 1000  || fabs(phoEta->at(phoCand[0]))>1.4442){phweight = 1;}
        if( phoEt->at(phoCand[0]) >= 1000  || fabs(phoEta->at(phoCand[0]))>1.4442){phweight = 1;}
        else{
	...
	//end tryinmg
        ...
        event_weight=event_weight*phweight; //trying
	...
	nPhoCand++;
		...
		fillHistos(30,event_weight,phoCand[0],jetveto,MET_to_use,uncorrectedPhoEt,METPhi_to_use);
                if(uncorrectedPhoEt/MET_to_use < 1.4){
                  nPhoEtOverMET_weighted += event_weight;
		  ...
		  fillHistos(21,event_weight_straightUp,phoCand[0],jetveto,MET_to_use,uncorrectedPhoEt,METPhi_to_use);
                  ...
                  fillHistos(23,event_weight,phoCand[0],jetveto,MET_to_use,uncorrectedPhoEt,METPhi_to_use);
                  ...
                  if(genWeight > 0.0) nPhoPToverMET_POS++;
                  else nPhoPToverMET_NEG++;
                }
    ...
    if(phoCandUp.size() > 0)
    {
      ...
      Float_t uncorrectedPhoEt = ((*phoEt)[phoCandUp[0]]);
        event_weight=1.0;
	Double_t phweight = 1.0;
        if( phoEt->at(phoCandUp[0]) >= 1000 || fabs(phoEta->at(phoCandUp[0]))>1.4442){phweight = 1;}
        else{
    ...
    if(phoCandDown.size() > 0)
    {
      ...
      Float_t uncorrectedPhoEt = ((*phoEt)[phoCandDown[0]]);
        event_weight=1.0;
	Double_t phweight = 1.0;
	if( phoEt->at(phoCandDown[0]) >= 1000 || fabs(phoEta->at(phoCandDown[0]))>1.4442){phweight = 1;}
        else{
  ...
  cout<<"nPhoEtOverMET_weighted: "<<nPhoEtOverMET_weighted; if(nDphiJetMET_weighted>0.){cout<<", "<<nPhoEtOverMET_weighted/nDphiJetMET_weighted<<" of previous";} cout<<endl;
  cout<<"Overall acc*eff: "<<nPhoEtOverMET_weighted/nInspected_genWeighted<<endl;
  cout<<endl;
  cout<<"nPhoPToverMET_POS: "<<nPhoPToverMET_POS<<endl;
  cout<<"nPhoPToverMET_NEG: "<<nPhoPToverMET_NEG<<endl;
}
  ...
  for(int i = 0; i < nJet; i++)
    {
      ...
      if(deltar>0.4 && jetPt->at(i) >30.0&& /*jetPUID->at(i)>value)*/(jetID->at(i) >>1&1==1))  
      {
