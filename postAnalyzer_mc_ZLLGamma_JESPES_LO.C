...
void postAnalyzer::Loop(Long64_t maxEvents, int reportEvery){
  ...
  double noGenPhotons, nUnder300, nOver300;
  noGenPhotons = nUnder300 = nOver300 = 0.0;
  double nPhoCand_weighted =0.0;
  int nPhoCand=0.0;
  ...
  for (Long64_t jentry=0; jentry<nentriesToCheck;jentry++){
    ...
    if(LHephoET == 0){
      noGenPhotons+=inspected_event_weight;
      continue;
    }

    /*    else if(LHephoET < 300){
	  nUnder300+=inspected_event_weight;
	  }
	  else{
	  nOver300+=inspected_event_weight;
	  continue;
	  }
    */
	...
	if(DeltaPhi(phoPhi->at(phoCand[0]),METPhi_to_use)>2.0)
	  {
    ...
    if(phoCandUp.size() > 0){
      //      Float_t uncorrectedPhoEt_up = ((*phoEt)[phoCand[0]]);
      event_weight=1.0;
      //      Float_t uncorrectedPhoEt = (*phoEt)[phoCand[0]];  
      Float_t uncorrectedPhoEt= phoEt->at(phoCandUp[0]);
	...
	if(DeltaPhi(phoPhi->at(phoCandUp[0]),METPhi_to_use)>2.0)
          {
	...
	if(DeltaPhi(phoPhi->at(phoCandDown[0]),METPhi_to_use)>2.0)
          {
  ...
  std::cout<<std::endl;
  std::cout<<"noGenPhotons: "<<noGenPhotons<<std::endl;
  std::cout<<"nOver300: "<<nOver300<<std::endl;
  std::cout<<"nUnder300: "<<nUnder300<<std::endl;
  std::cout<<std::endl;
}
...
Double_t postAnalyzer::NNLOCorrection(Double_t pt){
  ...
  if(pt >= 700.0 && pt <1000.0 ) corr =1.21;
  return corr;
}
...
Double_t postAnalyzer::NNLOCorrection_err(Double_t pt){
  ...
  if(pt >= 700.0 && pt <1000.0 ) corr = 1.0626 ;
  return corr;
}
